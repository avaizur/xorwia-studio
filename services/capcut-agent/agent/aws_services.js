...
- "fix" (complete fixed code, correctly escaped as a JSON string)
- "explanation" (what was wrong)
Return ONLY strictly valid JSON. Escape all newlines in code fields with \\n.`;
    ...
    try {
        ...
        const contentText = jsonResponse.content[0].text;
        const jsonMatch = contentText.match(/\{[\s\S]*\}/);
        if (!jsonMatch) return { issue: "Failed to parse AI output." };
        
        let rawJSON = jsonMatch[0];
        // Safely try to escape unescaped newlines within string values if Claude messed up
        try {
            return JSON.parse(rawJSON);
        } catch (initialParseError) {
            // Attempt to repair newlines inside string values:
